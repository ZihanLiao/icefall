...
def generate_lexicon(
    words: List[str],
    cn_lexicon_mapping: Dict[str, List[str]],
    token_sym_table: Dict[str, int],
) -> Lexicon:
    pattern = re.compile(r"([\u4e00-\u9fa5]+)")
    lexicon = []
    for word in words:
        if pattern.fullmatch(word.strip("\t")) is not None:
            print(word)
            tokens = cn_lexicon_mapping.get(word, None)
            if not tokens:
                print(f"word {word} does not have corresponding pinyin")
                continue
            tokens = list(tokens)
            if contain_oov(token_sym_table, tokens): continue
...
def generate_tokens(
    ...
    is_all_chinese = re.compile(r"([\u4e00-\u9fa5]+)")
    with open(text_file, "r", encoding='utf-8') as f:
        for line in f:
            line = re.sub(whitespace, "", line)
            words = line.split(" ")
            for word in words:
                if is_all_chinese.fullmatch(word) is not None:
                    pinyin = cn_lexicon_mapping.get(word, None)
                    if pinyin == None: continue
                    for each in pinyin:
                        if each not in tokens:
                            tokens[each] = len(tokens)
                elif is_all_english(word):
                    for char in word:
                        if char not in tokens:
                            tokens[char] = len(tokens)
                else:
                    logging.warning(f"WARNING: {word}")
                    continue
    return tokens
...
def main():
    lang_dir = Path("lang_pinyin_bpe")
    text_file = lang_dir / "text.seg"
    bpe_model = lang_dir / "bpe.model"
    words_file = lang_dir / "words.txt"
    cn_lexicon_file = "data/dict/cn_lexicon.txt"
    ...
    cn_lexicon_mapping = {}
    with open(cn_lexicon_file, 'r') as f:
        for line in f.readlines():
            splits = line.rstrip().split()
            word = splits[0]
            tokens = splits[1:]
            cn_lexicon_mapping[word] = tokens
    
    token_sym_table = generate_tokens(str(text_file), cn_lexicon_mapping)
    
    lexicon = generate_lexicon(words, cn_lexicon_mapping, token_sym_table)
    # lexicon = generate_lexicon(text_file, cn_lexicon_mapping, bpe_model)
    
    # tokens: Dict[str, int] = dict()
    # tokens["<blk>"] = 0
    # tokens["<sos/eos>"] = 1
    # tokens["<unk>"] = 2
    # for i, token in enumerate(list(set(lexicon.values()))):
    #     tokens[i+3] = token
